...
int main(void)
{
	...
	int int_size;
	int iArray_size;
	int iArray_num_elements,iArray_width,iArray_heigth,iArray_depth;
	int i,j,k;
	...
	iArray_heigth =  sizeof(iArray[0]) / sizeof(iArray[0][0]);
	printf("Number Of Rows (Heigth) In Three Dimensional ( 3D ) Integer Array Is = %d\n\n",iArray_heigth);
	
	iArray_depth = sizeof(iArray[0][0]) / int_size;
	printf("Depth In Three Dimensional ( 3D ) Integer Array Is = %d\n\n",iArray_depth);
	
	iArray_num_elements = iArray_width * iArray_heigth * iArray_depth;
